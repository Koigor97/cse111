...
def print_death_age(people_dict):
    ...
    for key, value in people_dict.items():
        ...
        print(f"{value[NAME_INDEX]} , age at death {age}")
...
def count_genders(people_dict):
    ...
    for key, value in people_dict.items():
        if "F" in value:
            ...
            male += 1
    print(f"Number of male: {male}, and number of female: {female}")
...
def print_marriages(marriages_dict, people_dict):
    ...
    Return: nothing
    """
    for people, people_value in marriages_dict.items():
        husband_detail = people_value[0]
        wife_detail = people_value[1]
        marriage_detail = people_value[2]

        man_list = people_dict[husband_detail]
        wife_list = people_dict[wife_detail]

        man_name = man_list[NAME_INDEX]
        man_birth = man_list[BIRTH_YEAR_INDEX]
        his_age = marriage_detail - man_birth

        woman_name = wife_list[NAME_INDEX]
        woman_birth = wife_list[BIRTH_YEAR_INDEX]
        her_age = marriage_detail - woman_birth

        print(f"{man_name} at {his_age} married > {marriage_detail} < {woman_name} at {her_age}")

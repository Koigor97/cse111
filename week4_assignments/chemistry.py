def main():
    the_formula = input("Enter the formula (make sure to enter it in the exact format): ")
    the_mass = float(input("Enter the chemical sample (grams): "))
    the_table = make_periodic_table()
    compound_list = parse_formula(the_formula, the_table)
    the_result = compute_molar_mass(compound_list, the_table)
    number_of_moles = the_mass / the_result

    known_molecule = make_known_molecules()
    the_name = get_formula_name(the_formula, known_molecule)
    the_proton = sum_protons(compound_list, the_table)

    print(f"The molar mass is: {the_result:.5f} grams/moles\n"
          f"The number of mole in the sample: {number_of_moles:.5f} moles")
    print(f"The name of the chemical: '{the_name.capitalize()}', with a total number of: {the_proton} proton")


def make_periodic_table():
    """This function contains all the 94 elements known to naturally occur on earth
    and returns a compound list as a value. This is the format: [symbol, name, atomic_mass]"""
    periodic_table_list = {
        "Ac": ["Actinium", 227],
        "Ag": ["Silver", 107.8682],
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    """Compute and return the molar_mass molar mass of all the
    ...
    Return: the molar_mass molar mass of all the elements in
    ...
    for i in range(len(symbol_quantity_list)):
        symbol = periodic_table_dict[symbol_quantity_list[i][SYMBOL_INDEX]]
        atomic_mass = symbol[ATOMIC_MASS_INDEX] * symbol_quantity_list[i][QUANTITY_INDEX]
        molar_mass += atomic_mass
    # Get the atomic mass for the symbol from the dictionary.
    # Multiply the atomic mass by the quantity.
    # Add the product into the molar_mass molar mass.
    return molar_mass
    # Return the molar_mass molar mass.
